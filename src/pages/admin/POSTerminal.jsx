...
import { ArrowLeft, ShoppingCart, Minus, Plus, Trash2, Send, CreditCard, Banknote, QrCode, Check, RefreshCw, X, Home, Lock, Shield, Loader2 } from 'lucide-react';
  ...
  const handleRazorpayPayment = async () => {
    ...
    try {
      ...
      const orderRes = await fetch('/api/razorpay/order', {
        ...
        body: JSON.stringify({ amount: combinedTotal }),
      });
      ...
      const options = {
        ...
        name: 'Odoo POS Cafe',
        description: `Table ${selectedTable?.number} — ${tableUnpaidOrders.length} order(s)`,
          ...
          try {
            ...
            if (verifyData.success) {
              ...
              tableUnpaidOrders.forEach(order => {
                dispatch({
                  type: 'UPDATE_PAYMENT_STATUS',
                  payload: { orderId: order.id, paymentMethod: 'razorpay' },
                });
              });
              ...
              <div className="payment-screen">
                ...
                <div className="payment-method-cards">
                  ...
                  {pm.razorpay && (
                    ...
                    >
                      <div className="payment-method-icon" style={{ fontSize: '1.5rem' }}>
                        <Shield size={28} color="#528FF0" />
                      </div>
                ...
                <AnimatePresence mode="wait">
                  ...
                  {paymentMethod === 'razorpay' && (
                    <motion.div key="razorpay" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} style={{ marginTop: '1.5rem' }}>
                      <div style={{ textAlign: 'center', padding: '2rem 1rem', background: '#eef2ff', borderRadius: 'var(--radius-md)' }}>
                        <Shield size={48} color="#528FF0" style={{ marginBottom: '0.75rem' }} />
                        <div style={{ fontSize: '1.25rem', fontWeight: 700, color: '#528FF0', marginBottom: '0.25rem' }}>
                          {formatCurrency(combinedTotal)}
                        </div>
                        <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)', margin: 0 }}>
                          {razorpayLoading ? 'Processing payment…' : 'Click "Validate Payment" to open Razorpay checkout'}
                        </p>
                        {razorpayLoading && (
                          <div style={{ marginTop: '1rem', display: 'flex', justifyContent: 'center' }}>
                            <Loader2 size={24} color="#528FF0" style={{ animation: 'spin 1s linear infinite' }} />
                          </div>
                        )}
                      </div>
                    </motion.div>
                ...
                {paymentMethod && (
                  <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1.5rem' }}>
                    ...
                    <button
                      ...
                      style={razorpayLoading ? { opacity: 0.6, cursor: 'wait' } : {}}
                    >
                      {razorpayLoading ? <Loader2 size={18} style={{ animation: 'spin 1s linear infinite' }} /> : <Check size={18} />}
                      {razorpayLoading ? 'Processing...' : paymentMethod === 'razorpay' ? 'Pay with Razorpay' : 'Validate Payment'}
                    </button>
                  </div>
                )}
              </div>
