...
function ensureCompleteState(dbState) {
  ...
  return {
    users: Array.isArray(dbState.users) ? dbState.users : defaults.users,
    ...
    orders: Array.isArray(dbState.orders) ? dbState.orders : defaults.orders,
  ...
  useEffect(() => {
    ...
    fetch('/api/state')
      ...
      .then(data => {
        ...
        const { products, users, floors } = data.state;
